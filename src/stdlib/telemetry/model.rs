...
/// An ISO-8601 / RFC-3339 UTC timestamp string from Unix nanoseconds, computed
/// without a `chrono` dependency (telemetry builds with only `data` + `net`).
/// Format: `YYYY-MM-DDTHH:MM:SS.mmmZ` (millisecond precision). PostHog accepts
/// this for the `timestamp` field. Uses the civil-from-days algorithm (Howard
/// Hinnant) for the date part.
pub fn iso_timestamp(unix_nanos: u128) -> String {
    let total_secs = (unix_nanos / 1_000_000_000) as i64;
