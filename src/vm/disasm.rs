...
pub fn disasm_at(chunk: &Chunk, offset: &mut usize) -> String {
    ...
    *offset = at + 1 + width;
    ...
    match op {
        ...
        Op::CallMethodSpread => {
            let idx = chunk.read_u16(at + 1);
            let _ = write!(line, "{idx:>5} ; .{}(...args)", const_repr(chunk, idx));
        }
        ...
        | Op::InstanceOfType | Op::MatchVariant | Op::VariantField | Op::MatchVariantHasField => {
            let idx = chunk.read_u16(at + 1);
            let _ = write!(line, "{idx:>5} ; {}", const_repr(chunk, idx));
        }
...
fn op_name(op: Op) -> &'static str {
    ...
    match op {
        ...
        RangeStepValue => "RANGE_STEP_VALUE",
        RangeResolveStep => "RANGE_RESOLVE_STEP",
        RangeHasNext => "RANGE_HAS_NEXT",
        Jump => "JUMP",
