...
# Recorded pre-SRV baseline for object_churn on the VM (bench/PROFILING_RESULTS.md,
# 2026-06-06, --profile profiling build). Gate 12 confirms the steady-state stays in
# the same band (machine + build-profile variance is ~10-15%).
GATE12_BASELINE_MS="4452"
...
lines.append(f"| pre-SRV baseline (PROFILING_RESULTS.md) | {baseline:.0f} |")
lines.append("")
lines.append(f"Delta vs baseline: **{delta_pct:+.1f}%** → **{verdict}** (within the ~±15%")
lines.append("machine + build-profile variance band; the added arm is predictably")
lines.append("not-taken for a non-Shared receiver). The three-way `vm_differential` gate")
lines.append("separately proves specialized == generic byte-identically over the corpus,")
lines.append("including the shared examples.")
lines.append("")
