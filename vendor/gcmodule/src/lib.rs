#![deny(missing_docs)]
// AScript vendoring: this is a verbatim copy of gcmodule 0.3.3 (plus the one wasm32
// `#[repr(C, align(8))]` fix on `GcHeader`), used via `[patch.crates-io]`. As a path dep it
// is now compiled+linted by our `cargo clippy --all-targets`, surfacing the upstream crate's
// own (harmless, pre-existing) warnings/lints. Silence them ALL — we do not maintain this
// third-party source; the only change vs upstream is the single alignment attribute, audited
// in `collect.rs`. (A registry dep is never linted; vendoring as a path dep exposes them.)
