...
impl<T: Trace> Cc<T> {
    ...
    /// Returns the strong reference count of this `Cc<T>`.
    ///
    /// LOCAL VENDOR ADDITION (not in upstream gcmodule 0.3.3). Exposes the
    /// crate-internal `ref_count()` (the count with the tracked/dropped metadata
    /// bits stripped) as a public accessor. Added for the AScript REGION spike's
    /// kill-site deadness proof (spec §3.3): a `strong_count() == 1` verdict means
    /// this is the SOLE owner of the underlying object (no live alias), the
    /// precondition for in-place object recycling. Holds for tracked Ccs (the
    /// metadata bits live below `REF_COUNT_SHIFT`, so the strong count is
    /// unaffected): fresh -> 1, `.clone()` -> 2, drop the clone -> 1.
    ///
    /// The PRODUCTION dependency decision (upstream PR vs official vendor, spec G6)
    /// is DEFERRED to a REGION GO outcome (Phase 3); this spike-local accessor does
    /// not ship.
