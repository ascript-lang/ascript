...
impl<T: ?Sized, O: AbstractObjectSpace> RawCcBox<T, O> {
    ...
    pub(crate) fn debug_name(&self) -> String {
        ...
        #[cfg(not(test))]
        {
            ...
            // AScript vendoring: the upstream `debug`+`nightly`-gated `optional_debug()`
            // call (and its `OptionalDebug` specialization trait below) were REMOVED — they
            // use `default fn` (the unstable `specialization` feature), which emits a
            // future-incompat `specialization is experimental` warning under our path-dep
            // clippy even though we never enable those features. The block is debug-only
            // value naming (a `{:?}` suffix on a tracked object); dropping it changes nothing
            // in any stable build (the `debug`/`nightly` features are off here and upstream).
