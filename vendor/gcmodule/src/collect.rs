...
///
/// AScript wasm32 fix (`align(8)`): `value()` computes the `CcBox` pointer as
/// `(self as *const Self).offset(1)` == `self + size_of::<GcHeader>()`, and
/// `RawCcBoxWithGcHeader { header, cc_box }` relies on that offset landing exactly on
/// `cc_box`. On a 32-bit target (wasm32) the three pointer fields make `GcHeader` 12 bytes,
/// but `RawCcBox<T>` is 8-aligned (it holds an 8-aligned `Value`), so `#[repr(C)]` inserts 4
/// bytes of padding before `cc_box` (it lands at offset 16) while `offset(1)` still computes
/// self+12 → a 4-byte-short box pointer → heap corruption (the cc.rs:175 `debug_assert`
/// 84 vs 88). Forcing 8-byte alignment makes `size_of::<GcHeader>()` == 16 on wasm32 so the
/// offset is correct. On 64-bit native the three 8-byte pointers already make it 24
/// (8-aligned), so `align(8)` is a NO-OP — the layout and behavior stay byte-identical
/// (proven by the unchanged native `vm_differential` 446/0 in both feature configs).
#[repr(C, align(8))]
pub struct GcHeader {
