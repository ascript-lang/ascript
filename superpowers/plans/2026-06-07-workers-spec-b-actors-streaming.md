...
- [ ] **Step 1: Failing unit test.** In `src/worker/isolate.rs` tests, add a test that `spawn_isolate` (the dedicated, non-pooled form) births a thread with its own runtime, runs a trivial closure shipped as bytes, returns a result over a `oneshot`, and that dropping the returned handle joins/aborts the thread cleanly (no zombie thread). Run → **expect FAIL** (dedicated spawner not yet public).

- [ ] **Step 2: Public dedicated spawner.** Ensure `src/worker/isolate.rs` exposes a `pub(crate) fn spawn_isolate(...) -> IsolateHandle` that does the Plan A bootstrap (OS thread + `current_thread` runtime + `LocalSet` + fresh `Interp`/`Vm` + `global_env`, `WORKER_STACK_SIZE` stack) but is **NOT pooled** — it lives until its `IsolateHandle` is dropped (cancel-on-drop: drop sends an abort + joins). If Plan A only has a pool-internal spawner, refactor the common bootstrap into `spawn_isolate` and have the pool call it. The actor (§5) and stream (§6) drivers both call this one function — the SHARED dedicated-isolate lifecycle.

- [ ] **Step 3: `IsolateHandle`.** Define the handle: the inbound `Send` byte channel (`tokio::sync::mpsc` of serialized messages), an `AbortHandle`/join handle, and a `Drop` impl that signals teardown and joins. The actor wraps it with a mailbox; the stream wraps it with a demand driver. Skeleton `actor.rs`/`stream.rs` that re-export `IsolateHandle`.

- [ ] **Step 4: Declare mods.** In `src/worker/mod.rs`, add `pub(crate) mod actor;`, `pub(crate) mod actor_handle;` (or fold into actor), `pub(crate) mod stream;`.

- [ ] **Step 5:** Run the isolate unit tests in both feature configs → **expect PASS**. Confirm `cargo clippy --all-targets` clean (watch `await_holding_refcell_ref`).
