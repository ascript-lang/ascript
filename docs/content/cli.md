...
| `ASCRIPT_EXECUTOR` | Set to `tokio` to force the per-isolate task driver back to stock tokio (the permanent kill switch for the bespoke executor). By default AScript drives its own tasks (spawn/wake/yield) on a bespoke single-thread executor; tokio remains the I/O + timer driver either way. Behavior is byte-identical; a debugging and differential knob, like `ASCRIPT_NO_SPECIALIZE`. |
