...
| `ASCRIPT_NO_DECODE_INLINE` | Set to `1` to disable DECODE Unit C (speculative global-fn inlining). Decoding and Unit B fusion continue; only inlining is suppressed. Behavior is byte-identical. |
| `ASCRIPT_NO_DECODE_TOS` | Set to `1` to disable DECODE Unit D (top-of-stack register caching in the record burst). Decoding, fusion, and inlining continue; only TOS caching is suppressed. Behavior is byte-identical. |
