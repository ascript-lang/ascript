...
| `ASCRIPT_NO_REGIONS` | Set to `1` to disable the REGION proven-dead object recycler (the permanent kill switch, mirroring `--no-specialize`). Only consulted in a build compiled with the dev-only `--features region-spike` recycler (compiled out of every shipped build); behavior is byte-identical to the recycler-on path — only allocation counts and throughput differ. A benchmarking knob with no effect on a default build. |
...
| `ASCRIPT_REGION_POOL_CAP` | Maximum pooled cells per isolate for the REGION proven-dead object recycler (default `256`). Only read in a build compiled with the dev-only `--features region-spike` recycler (compiled out of every shipped build); a recycle into a full pool falls through to the normal drop. A benchmarking knob with no effect on a default build. |
| `ASCRIPT_REGION_PROBE_OUT` | Path the REGION allocation-lifetime probe appends its histogram (one JSON line) to at program end. Only read in a build compiled with the dev-only `--features region-probe` instrument (compiled out of every shipped build); absent or empty → no output. A research/benchmarking knob with no effect on a default build. |
