...
- [x] Implemented in `src/stdlib/http_server.rs` (feature `net`). **Decision: hand-rolled HTTP/1, NOT hyper's `Service`.** hyper's `serve_connection` owns the response future, which can't borrow `&mut Interp` (the interp is `!Send`, current-thread). So the accept loop parses the HTTP/1 request line + headers + body directly off the `tokio::net::TcpStream`, dispatches through `self.call_value(handler, [reqObj], span).await`, and writes the response bytes back — keeping `&mut self` available the whole cycle. `Value::Native(kind=HttpServer)` holds routes/middleware/listener in `ResourceState::HttpServer`.
  - **Testable lifecycle:** `bind(host,port)→[boundPort,err]` binds without looping (read the OS-assigned port from port 0); `serve(opts?)→[nil,err]` runs the accept loop, `opts.maxRequests:N` returns after N requests; `listen(host,port,opts?)` = bind+serve. (Chose `bind`+`serve`+`maxRequests` — both an ephemeral-port read AND a deterministic stop.)
  - **Middleware:** `use((req,next)=>resp)`; `next` is a single-use callable (an `HttpNext` native handle) carrying the chain continuation. Calling `next(req?)` advances to the next middleware (or the matched route handler); returning without calling `next` short-circuits. Un-consumed continuations are swept after each request (no per-request leak).
  - **Response conversion:** string→200 text/plain; object `{status?,headers?,body?}`; `[value,err]` pair (non-nil err→500). Request object: `{method,path,query,headers(lowercased),params,body(string)}`.
  - **Robustness (Task 6 review):** a handler/middleware panic (`Control::Panic`) or `?`-propagation (`Control::Propagate`) is caught in `dispatch_request` and converted to a **500** (message included) so the accept loop survives — NOT propagated out of `serve`. Request limits bound hostile clients: `MAX_HEADER_BYTES` 64 KiB → 431; `maxBodySize` (default 16 MiB, serve-opt configurable) → 413 (declared CL checked, body not read); `requestTimeout` (default 30s, serve-opt configurable, ms) wraps the read via `tokio::time::timeout` → 408 on expiry. These matter doubly under sequential handling (one stuck/oversized request would otherwise stall the whole server).
  - **v1 limitation (documented in the module + here):** requests are handled STRICTLY SEQUENTIALLY — one request per connection, handler awaited fully before the next accept. Concurrent connection handling is deferred (correct under a single `&mut Interp`). Each connection is closed after one response (`connection: close`).
- [x] Tests (13, in `http_server.rs`): GET returns body; path param `/users/:id`; POST body echoed; object response `{status:201, headers, body}`; 404 unmatched; middleware short-circuits with 401; middleware calls `next` → handler runs; middleware adds a header; query params parsed; **AScript std/net/http client → AScript server end-to-end** (client on its own current-thread runtime in a spawned OS thread); short-circuit leaves no leaked handles; **handler panic → 500 and loop survives a 2nd request** (`maxRequests:2`); **oversized body → 413 and server survives** (`maxBodySize:10`). Server runs inline (interp `!Send`), raw/AScript client drives it with `maxRequests`. Commit `feat: std/http/server (hyper/hand-rolled HTTP1, routes/handlers/middleware/params, sequential)` + `fix: http server catches handler panics (->500, loop survives); add header/body size limits + read timeout (Task 6 review)`.
...
- [x] `examples/net.as`: self-contained TCP loopback echo (no external network). `listen("127.0.0.1",0)` → read `server.port` → `await connect(...)` (handshake completes into the OS listen backlog) → `await server.accept()` (dequeues it) → `client.write("ping\n")` → `conn.readLine()` → `conn.write("pong\n")` → `client.readLine()`. Single-threaded sequencing runs deterministically (no deadlock) — verified on macOS. Output: `nil\nnil\nnil\nping\npong`.
- [x] `runs_net_example` in tests/cli.rs, gated `#[cfg(all(feature="net", unix))]`, asserts the round-trip output `"nil\nnil\nnil\nping\npong\n"`.
- [x] **`http3` feature (default-off):** added `http3 = ["net", "reqwest/http3"]` (NOT in `default`). The `httpVersion:"3"` arm is cfg-gated: `#[cfg(feature="http3")]` → `b.http3_prior_knowledge()`; `#[cfg(not(feature="http3"))]` → clean Tier-1 error "HTTP/3 requires the 'http3' build feature". `cargo build` (default, no http3) compiles. Enabling `http3` additionally needs `RUSTFLAGS=--cfg reqwest_unstable` (documented in Cargo.toml + net_http.rs). The clean-error test is `#[cfg(not(feature="http3"))]`.
- [x] **Deferrals doc:** §11.5 deferrals documented in the `src/stdlib/net_http.rs` module doc comment: HTTP/3 (feature-gated, default-off, needs reqwest_unstable), response trailers (best-effort — reqwest's high-level API exposes no `resp.trailers`; trailing headers dropped), SOCKS proxy (SUPPORTED via reqwest `socks` feature, compiles cleanly — listed only because it ships behind that feature).
- [x] Conformance: example parses under both parsers (frontend + tree-sitter). FINAL: `cargo test` (default) + `cargo test --no-default-features` (net cfg's out) + `cargo clippy --all-targets` (both) + `cargo build --no-default-features` + `cargo build` (default). All green/clean/compile. Commit `test: net loopback example + http3 feature gate + §11.5 deferrals doc`.
