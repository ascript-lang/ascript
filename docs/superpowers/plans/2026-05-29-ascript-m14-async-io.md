...
- [x] Tests (11, in `http_server.rs`): GET returns body; path param `/users/:id`; POST body echoed; object response `{status:201, headers, body}`; 404 unmatched; middleware short-circuits with 401; middleware calls `next` → handler runs; middleware adds a header; query params parsed; **AScript std/net/http client → AScript server end-to-end** (client on its own current-thread runtime in a spawned OS thread); short-circuit leaves no leaked handles. Server runs inline (interp `!Send`), raw/AScript client drives it with `maxRequests`. Commit `feat: std/http/server (hyper/hand-rolled HTTP1, routes/handlers/middleware/params, sequential)`.
