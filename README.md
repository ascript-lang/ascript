...
| Networking | `std/net` (DNS: `lookup` · `lookupOne`) · `std/net/tcp` · `std/net/udp` (datagram sockets) · `std/net/http` · `std/http/server` (verb methods: `get`/`post`/`put`/`patch`/`delete`/`head`/`options`; schema-validated typed routes; **multi-core** via `serve({ workers: N })` — `SO_REUSEPORT` accept loops across N isolates, single-isolate Windows fallback) · `std/net/ws` |
