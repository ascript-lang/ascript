...
| Concurrency | `std/task` (`spawn` · `gather` · `race` · `timeout` · `retry` · `pipe` over `future<T>`) · `std/sync` (FIFO channels · counting semaphore · token-bucket rate limiter) · shared-nothing `worker fn`/`worker class`/`worker fn*` for multi-core parallelism |
