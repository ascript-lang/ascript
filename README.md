...
- **Async & concurrency (single-threaded per isolate, multi-core via shared-nothing workers)** — `await` any I/O on a cooperative event loop; `future<T>` and `std/task` (`spawn`/`gather`/`race`/`timeout`/`retry`/`pipe`); structured concurrency with cancel-on-drop; `std/sync` for channels, semaphores, and rate limiters; `std/time` `interval`/`debounce`/`throttle` timer utilities. No data races. For multi-core parallelism, **shared-nothing workers** run on separate isolates with only deep-copied data crossing: `worker fn`/`static worker fn` (pooled, stateless, return `future<T>`), `worker class` actors (stateful, dedicated isolate, async proxy), and `worker fn*` streaming generators (`for await`).
...
| Data & serialization | `std/json` · `std/csv` · `std/toml` · `std/yaml` (all with typed `parse(text, Class\|schema)`) · `std/msgpack` · `std/cbor` (binary) · `std/encoding` · `std/regex` · `std/uuid` · `std/url` · `std/xml` (strict XML 1.0 parse/stringify) · `std/html` (escape/unescape · fail-closed allowlist `sanitize`) · `std/decimal` (exact 96-bit decimal arithmetic) |
...
| System & files | `std/fs` · `std/env` · `std/io` · `std/process` · `std/crypto` · `std/compress` (gzip/deflate/zip · zstd · brotli · tar) · `std/archive` (streaming tar/zip writers + lazy entry generators · gzip-tar · `deterministic` reproducible output · hardened zip-slip-safe disk extraction) · `std/sqlite` |
| Databases | `std/postgres` · `std/redis` (async network clients; native-resource handles) |
| Mail & object storage | `std/email` (RFC 5322 message builder — `multipart/alternative`/`mixed`, RFC 2047 encoded-words, header-injection-proof — + a hand-rolled SMTP client: EHLO/STARTTLS/AUTH/`send`) · `std/blob` (S3-compatible object storage over AWS SigV4: `put`/`get`/`head`/`delete` · lazy `list` · `presign` · streaming `putMultipart`; works against S3, R2, MinIO, B2) |
...
| Networking | `std/net` (DNS: `lookup` · `lookupOne`) · `std/net/tcp` · `std/net/udp` (datagram sockets) · `std/net/unix` (UDS client + listener, structural mirror of tcp) · `std/net/http` (`{socketPath}` over UDS shares the same response surface) · `std/http/server` (verb methods: `get`/`post`/`put`/`patch`/`delete`/`head`/`options`; schema-validated typed routes; **multi-core** via `serve({ workers: N })` — `SO_REUSEPORT` accept loops across N isolates, single-isolate Windows fallback; graceful drain: `srv.shutdown()` + `drainTimeout`) · `std/net/ws` |
| Containers | `std/docker` (typed Docker Engine API client over UDS: `connect` · `containers`/`inspect`/`create`/`start`/`stop`/`restart`/`wait`/`remove`/`images`/`removeImage`/`ping`/`version`/`info` · `logs`/`events`/`pull` as `for await` streams with auto-detect multiplex demux · `exec` convenience (create→start→drain→inspect) · `execCreate`/`execStart`/`execInspect` low-level steps; dual-cap gate: requires `net` ∧ `process`; Unix-only) |
| Concurrency | `std/task` (`spawn` · `gather` · `race` · `timeout` · `retry` · `pipe` · `pmap`/`preduce` (data parallelism across cores) over `future<T>`) · `std/sync` (FIFO channels · counting semaphore · token-bucket rate limiter) · shared-nothing `worker fn`/`worker class`/`worker fn*` for multi-core parallelism |
| Resilience | `std/resilience` (circuit breaker · token-bucket limiter · keyed per-client limiter · bulkhead/concurrency cap · retry with `retryOn`/`retryIf` · memoize with stampede protection · singleflight · fallback · deadline propagation · trace id propagation · health checks · Prometheus metrics; per-isolate; composable by plain nested calls) |
| Auth | `std/jwt` (JSON Web Tokens with **typed keys** that structurally kill alg-confusion: `hmacKey` · `sign` · `verify` · `decode`; HS256/384/512 over `hmac`+`sha2`; `alg:"none"` rejected in any casing; the header `alg` is intersected with the key kind's algorithm set ∧ the caller's allowlist; `jku`/`jwk`/`kid` headers ignored; constant-time signature verify; `auth` feature) · `std/oauth` (OAuth 2.0 Authorization Code + **PKCE S256** — `authorizeUrl`/`exchange`/`refresh` over the pooled HTTP client; `auth` feature) |
| Shared heap | `std/shared` (`freeze` · `isShared`: an immutable, `Arc`-backed read-only snapshot — the runtime's first `Send` value, shared across worker isolates by an `Arc` pointer bump, not a per-dispatch deep copy; mutation is a frozen-value panic) |
...
| Toolbelt | `std/cron` (5-field Vixie cron: `parse`/`next`/`nextN`/`matches`/`schedule`; DOM/DOW OR rule; fixed-offset TZ; replay-deterministic scheduling under the det clock) · `std/semver` (SemVer 2.0.0 `parse`/`valid`/`compare`/`sort` + node-semver-subset `satisfies`/`maxSatisfying`/`minSatisfying`: caret/tilde/x-range/hyphen/`\|\|`) · `std/markdown` (CommonMark → HTML over `pulldown-cmark`, **sanitized by default** + GFM tables/strikethrough/task-lists; `render`/`escape`) · `std/diff` (hand-rolled Myers O(ND) line/char diff + `unified` patch output; wires into `assert.snapshot` failure diffs) |
