...
fn multimodule_archive_has_both_modules() {
    let (arch, _report) = compile_archive(Path::new("examples/bundle_multimodule.as"), false, false)
...
fn logical_keys_are_machine_independent() {
    let (arch, _report) = compile_archive(Path::new("examples/bundle_multimodule.as"), false, false).expect("archives");
...
fn parent_directory_import_keys_with_dotdot() {
    ...
    let (arch, _report) = compile_archive(&entry, false, false).expect("archives a parent-dir import");
...
fn diamond_import_dedups_shared_module() {
    ...
    let (arch, _report) = compile_archive(&entry, false, false).expect("archives the diamond");
    assert_eq!(
        arch.modules.len(),
        4,
...
fn circular_import_terminates_and_archives_both() {
    ...
    let (arch, _report) = compile_archive(&a, false, false).expect("compile_archive terminates on a cycle");
...
fn unknown_package_is_a_clean_error() {
    ...
    .expect("write");
    let err = compile_archive(&entry, false, false).expect_err("unknown package must error");
    assert!(
...
fn missing_import_is_a_clean_error() {
    ...
    let err = compile_archive(&entry, false, false).expect_err("missing module must error");
...
fn single_module_archive_has_one_entry() {
    ...
    let (arch, _report) = compile_archive(&entry, false, false).expect("archives a zero-import program");
...
fn std_imports_are_not_archived() {
    ...
    let (arch, _report) = compile_archive(&entry, false, false).expect("archives, skipping std");
...
async fn archive_run_survives_deleted_source_tree() {
    ...
    let (arch, _report) = compile_archive(&entry, false, false).expect("archives");
...
async fn circular_archive_runs_once_no_infinite_loop() {
    ...
    let (arch, _report) = compile_archive(&main, false, false).expect("archives the cycle");
    let bytes = arch.encode();
    drop(dir);
    let arch = ModuleArchive::decode(&bytes).expect("decodes");
...
async fn nested_and_parent_dir_imports_resolve_from_archive() {
    ...
    let (arch, _report) = compile_archive(&entry, false, false).expect("archives nested + parent imports");
...
async fn assert_shaken_equals_unshaken(entry: &Path) -> ascript::compile::shake::ShakeReport {
    let (shaken_arch, report) =
        ascript::compile_archive_with_shake(entry, false, true, false).expect("shaken archive builds");
    let (unshaken_arch, _unshaken_report) =
        ascript::compile_archive_with_shake(entry, false, false, false).expect("unshaken archive builds");
...
async fn differential_side_effectful_toplevel_in_source_order() {
    ...
    let (arch, _r) = compile_archive(&entry, false, false).expect("archives");
